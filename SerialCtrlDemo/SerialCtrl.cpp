...
{
	char buffer[250],buffer1[250];
    CFileDialog dlgFile(FALSE ,NULL,NULL,OFN_OVERWRITEPROMPT,_T("Télé Information (*.tel)|*.tel|All Files |*.*||")); 
	
	dlgFile.DoModal();
	sprintf(&buffer[0],"%s",dlgFile.m_ofn.lpstrFile);
	sprintf(&buffer1[0],"%s",portName);

	if (m_portStatus == FALSE)  // if port is opened already, not open port again.
	{
		fopen_s(&pt_fichier,(LPCSTR )&buffer[0],"wb");

		m_portHandle = CreateFile((LPCSTR) &buffer1[0],  // Specify port device: default "COM1"
		...
		{
			AfxMessageBox(_T("Get configuration port has problem."),MB_OK);
			return FALSE;
		}
		...
		{
			AfxMessageBox(_T("Problème de configuration du port"),MB_OK);
			return FALSE;
		}
...
BOOL SerialCtrl::Write(const char * outputData, const unsigned int & sizeBuffer, unsigned long & length)
{
	if (length > 0)
	{
		...
		{
			AfxMessageBox(_T("Problème de lecture sur le port"),MB_OK);
			return FALSE;
		}
		return TRUE;
	}
...
BOOL SerialCtrl::ClosePort(void)
{
	if (m_portStatus == TRUE)               // Port need to be open before.
	{
		...
		{
			AfxMessageBox(_T("Problème de fermeture du port"));
			return FALSE;
		}    
		return TRUE;
	}
...
int SerialThread::Run()
{
	...
	sprintf(&buffer[0],"%s",m_serialIO->m_strPortName);
	...
	while(1)
	{
		...
		while(m_serialIO->GetProcessActivateValue()==TRUE)
		{
			...
			{
				if(serialCtrl().OpenPort(m_serialIO->m_DCB,&buffer[0])==TRUE)
				{
...
BOOL CSerialIO::Init()
{
	...
	int temp;
	sprintf(&buffer[0],"%s",m_strBaudRate);
	temp = atoi(buffer);
